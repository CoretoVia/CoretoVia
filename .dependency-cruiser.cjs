...
module.exports = {
  forbidden: [
    /**
     * ДЛ-Т6 · ход 9 (11.09.2026) · ДОСТИЖИМОСТ, не само пряк внос.
     *
     * Четири забрани долу обещаваха повече, отколкото броят: „само чете" (Огледалото),
     * „не държи Врата" (командите), „през Портата" (Книгата), „САМО main.ts" (екранът)
     * — а нито един техен регекс не лови `src/yadro/index.ts`, който преизнася
     * Вратата, нито преизнасяне през друг модул. Доказано с дърво (`tests/sloevete.test.ts`):
     * прозорец, който взима `Vrata` от барела или от `src/porta/preiznasya.ts`, минаваше
     * с нула нарушения. Това правило пита ДОСТИГА ли се Вратата по какъвто и да е път
     * (`reachable: true`, типовите ребра включително) от всичко извън корена
     * (`app/main.ts`), изпълнителя, носителя и ядрото.
     */
    {
      name: 'nikoy-ne-stiga-do-vratata',
      comment:
        'До Вратата стигат само app/main.ts, изпълнителят, носителят и ядрото — по никой път, барел и преизнасяне включително (ДЛ-Т6).',
      severity: 'error',
      from: { path: '^(app/(?!main\\.ts$)|src/(?!porta/izpalnitel\\.ts$|nositel/|yadro/))' },
      to: { path: '^src/yadro/vrata\\.ts$', reachable: true },
    },
    {
      name: 'bez-krag',
