...
const POMOSHT_NA_GLAVATA = pomosht(
  'Натискаш главата и редовете се подреждат по тази колона · второто натискане обръща посоката, третото връща реда от Книгата. Десният ѝ ръб се влачи и мести ширината, както в Ексел.',
  'натискане · подредба нагоре, надолу, както е · ръбът мести ширината',
);
...
export function glavaSPodredbaHTML(koloni: readonly Kolona[]): Zapechatan {
  return h`<tr>
      ...
      ${koloni.map(
        (k) =>
          h`<th data-kolona="${k.klyuch}" class="${k.vid}"${podskazka(k.pomosht)}><button type="button" class="glava-podredba" data-podredi="${k.klyuch}"${podskazka(POMOSHT_NA_GLAVATA)}>${k.ime}<span class="strelka" data-strelka="${k.klyuch}"></span></button><span class="drazhka" data-shirina="${k.klyuch}" aria-hidden="true"></span></th>`,
      )}
    </tr>`;
}
