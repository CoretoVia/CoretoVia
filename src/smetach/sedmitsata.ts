 ...
 *
 * ОТВОРЕНО И КАЗАНО: „докато не се отбележи, че е свършена" иска ОТМЕТКА върху
 * задачата, а такава колона в Модела още няма. Дотогава трупащата се задача е
 * онази с МИНАЛ край — вярно за всичко, което наистина не е свършено, и
 * прекалено за онова, което е свършено, но никой не го е казал на програмата.
 * Това е ЕДИНСТВЕНАТА разлика от думата му и стои написана, вместо да мълчи.
 */
...
export function sedmichnataPrograma(o: Ogledalo, chovek: string, dnes: string): SedmichnaPrograma {
  ...
  if (tv !== undefined) {
    for (const i of zhiviteRedove(tv)) {
      ...
      // МИНАЛ край · задачата се трупа на ДНЕС и стои там, докато не се затвори
      if (kray < dnes) {
        natrupani += 1;
